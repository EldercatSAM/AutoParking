...
background = pygame.image.load(r"C:\Users\Sam\Documents\School\BJUT\Classes\智能控制\倒车入库\background_640x480.jpg").convert()

# 坦克精灵类
tank_image = pygame.image.load(r'C:\Users\Sam\Documents\School\BJUT\Classes\智能控制\倒车入库\tank.png').convert_alpha()
cannon1_image = pygame.image.load(r'C:\Users\Sam\Documents\School\BJUT\Classes\智能控制\倒车入库\cannon_1.png').convert_alpha()

class FuzzyController():
    def __init__(self):
        self.dest_x,self.dest_y = 320,480
        self.xy_min,self.xy_max = 0.25,4
        self.xylow_max,self.xyhigh_min,self.xyhigh_max = 0.5,2,15
        self.a_min, self.a_max, self.alow_max,self.ahigh_min,self.ahigh_max = 0,90,20,70,90
    ...
    def Fxy(self, x, y):
        ...
        def fxyl(tmp):
            ...
            if tmp > self.xyhigh_max:
                ...
                res['xyhigh'] = (tmp - 1) / (self.xyhigh_max - self.xyhigh_min) 
        ...
        def fal(tmp):
            ...
            if tmp < self.alow_max:
                res['alow'] = tmp / self.alow_max

            if tmp > self.a_min and tmp < self.a_max:
                if tmp >= 45:
                    res['amidian'] = -(tmp - 45) / (self.a_max - 45) + 45
                else:
                    res['amidian'] = (tmp - self.a_min) / (45 - self.a_min)

            if tmp > self.ahigh_max:
                res['ahigh'] = 1
                return res
            elif tmp > self.ahigh_min:
                res['ahigh'] = (tmp - 45) / (self.ahigh_max - self.ahigh_min) 
...
class HeroTank(pygame.sprite.Sprite):
    ...
    def update(self):
        self.controller_angle = self.fuzzyController.control(self.current_x, self.current_y)
